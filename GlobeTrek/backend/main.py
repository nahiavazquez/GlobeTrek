...
async def oauth_callback(code: str, db: Session = Depends(get_db)):
    ...
    if existing_user:
        ...
        return RedirectResponse(url=f"{settings.front_baseUrl}/GlobeTrek-app/#/en/home?token={token}&id={existing_user.id}")
    else:
        ...
        return RedirectResponse(url=f"{settings.front_baseUrl}/GlobeTrek-app/#/en/new_password?token={token}&id={new_user.id}")
